...
func TestVerifyCertExtensionsCustomizedIssuer(t *testing.T) {
	...
	t.Run("VerifyCertExtensions with partial issuer match", func(t *testing.T) {
		...
		require.ErrorContains(t, err, "expected Issuer to be https://token.actions.githubusercontent.com, got https://token.actions.githubusercontent.com/foo-bar -- if you have customized")
	})
